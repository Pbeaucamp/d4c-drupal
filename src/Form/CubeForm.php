...
class CubeForm extends HelpFormBase {
	...
	public function submitForm(array &$form, FormStateInterface $form_state){
		...
		drupal_set_message("cd /home/user-client && java -cp d4cmetadata.jar bpm.metadata.tools.D4CCube -u \"" . $urlCkan . "\" -k \"" . $cle . "\" -o \"infogreffe\" -j \"jdbc:postgresql://" . $connection['default']['host'] . ":" . $connection['default']['port'] . "/" . str_replace('drupal_d4c', 'datastore', $connection['default']['database']) . "\" -l \"" . $connection['default']['username'] . "\" -p \"" . $connection['default']['password'] . "\" -n \"" . $itemName . "\" -d " . $id . " -up " . $up . " -ds \"" . $string_dataset_lies . "\" -dim \"" . $dimensions . "\" -mes \"" . $measures . "\" > cubelogs.txt");
