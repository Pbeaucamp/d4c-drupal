	...
	(this, (function () {
			...
			function init() {
				...
				function focusTriggersKeyboardModality(el) {
					console.log("focusTriggersKeyboardModality");
					var type = el.type;
...
(function (window, document, undefined) {
					...
					for (var i = 0, ii = directives.length; i < ii; i++) {
						...
						if (directiveValue = directive.transclude) {
							...
							if (directiveValue == 'element') {
								...
								$compileNode = templateAttrs.$$element = jqLite(document.createComment(' ' + directiveName + ': ' +
	...
	function getStringValue(name, fullExpression) {
		name = name + '';
		if (!isString(name)) {
			throw $parseMinErr('iseccst', 'Cannot convert object to primitive value! '
				 + 'Expression: {0}', fullExpression);
		}
		return name;
	}
	...
	function ensureSafeAssignContext(obj, fullExpression) {
		if (obj) {
			if (obj === (0).constructor || obj === (false).constructor || obj === ''.constructor || obj === {}
				.constructor || obj === [].constructor || obj === Function.constructor) {
...
(function (factory) {
	...
	angular.module('ng-sortable', []).constant('ngSortableVersion', '0.4.1').constant('ngSortableConfig', {}).directive('ngSortable', ['$parse', 'ngSortableConfig', function ($parse, ngSortableConfig) {
				var removed,
				nextSibling;
				...
				return {
					...
					compile: function ($element, $attr) {
						...
						var match = getNgRepeatExpression(ngRepeat).match(/^\s*([\s\S]+?)\s+in\s+([\s\S]+?)(?:\s+as\s+([\s\S]+?))?(?:\s+track\s+by\s+([\s\S]+?))?\s*$/);
						if (!match) {
							return;
						}
						var rhs = match[2];
						return function postLink(scope, $el) {
							...
							angular.forEach(['sort', 'disabled', 'draggable', 'handle', 'animation', 'group', 'ghostClass', 'filter', 'onStart', 'onEnd', 'onAdd', 'onUpdate', 'onRemove', 'onSort', 'onMove', 'onClone', 'setData'], function (name) {
...
}); ; /*! js-cookie v2.0.4 | MIT */!function (a) {
	...
	function b(c) {
		function d(b, e, f) {
			...
			for (var j = document.cookie ? document.cookie.split("; ") : [], k = /(%[0-9A-Z]{2})+/g, l = 0; l < j.length; l++) {
				...
				try {
					if (o = c && c(o, n) || o.replace(k, decodeURIComponent), this.json)
						try {
							o = JSON.parse(o)
						} catch (i) {}
					if (b === n) {
